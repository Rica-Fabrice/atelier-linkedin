...
export function Testimonials({ alumniTitle, alumniTestimonials }: TestimonialsProps) {
  ...
  return (
    <section id="alumni" className="bg-background py-24 sm:py-32 border-b border-white/[0.05]">
        ...
        <div className="mt-10 sm:mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {alumniTestimonials.map((t: Testimonial, i: number) => (
            <Reveal key={i} delay={0.1 + i * 0.1} className="h-full">
              <div className="h-full flex flex-col rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-4 sm:p-6 text-left transition-transform hover:-translate-y-1 hover:shadow-2xl hover:shadow-foreground/5">
